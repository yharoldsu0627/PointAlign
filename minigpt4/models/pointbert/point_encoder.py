...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts, return_pos_and_output=False):
        ...
        pos_with_cls = torch.cat((cls_pos, pos), dim=1) 
        
        # transformer
        x = self.blocks(x, pos_with_cls)
        x = self.norm(x)  
        if return_pos_and_output:
            return pos_with_cls, x  
